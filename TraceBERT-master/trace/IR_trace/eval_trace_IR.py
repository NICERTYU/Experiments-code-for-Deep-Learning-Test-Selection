...
from code_search.IR_baseline.IRs import VSM, LDA, LSI, BM25Similarity, LevenshteinDistance, JaccardSimilarity, BERTBaseSimilarity, Word2VecSimilarity, GloVeSimilarity, BERTSimilarity
...
if __name__ == "__main__":
    ...
    if args.model_path == "VSM":
        ...
        model.build_model(doc_tokens)
    elif args.model_path == "Word2Vec":
     
        model = Word2VecSimilarity()
        model.build_model(doc_tokens)
    elif args.model_path == "Jaccard":
      
        model = JaccardSimilarity()
        model.build_model(doc_tokens)
    elif args.model_path == "Levenshtein":
        model = LevenshteinDistance()
        model.build_model(doc_tokens)
    elif args.model_path == "Bert":
        model = BERTSimilarity("./bert-base-uncased")
        model.build_model(doc_tokens)
        print("-----------------------------IIOOIOIIO___________")
    else:
