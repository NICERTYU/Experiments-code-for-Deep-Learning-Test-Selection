...
#返回字典， key为commit_id, value为commit
...
def __save_artifacts(art_list, type, output_file):
    ...
    for art in art_list:
        if type == "issue" or type == "commit":
            new_row = pd.DataFrame([art.to_dict()])
        elif type == "link":
            new_row = pd.DataFrame([{"issue_id": art[0], "commit_id": art[1]}])
        else:
            raise Exception("wrong artifact type")
...
if __name__ == "__main__":
    ...
    output_dir = '/Users/gw/Desktop/Experiments/TraceBERT-master/TBERT git projects'
