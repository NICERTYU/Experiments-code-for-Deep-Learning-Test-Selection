...
class RelationClassifyHeader1(nn.Module):
    """
    H2:
    use averaging pooling across tokens to replace first_token_pooling
    """
...
class RelationClassifyHeader3(nn.Module):
    """
    H2:
    use averaging pooling across tokens to replace first_token_pooling
    """
    ...
    def forward(self, code_hidden, text_hidden):
        pool_code_hidden = self.code_pooler(code_hidden)
        pool_text_hidden = self.text_pooler(text_hidden)
        multi_hidden = torch.mul(pool_code_hidden,pool_text_hidden)
        
        concated_hidden = multi_hidden
...
class RelationClassifyHeader4(nn.Module):
    """
    H2:
    use averaging pooling across tokens to replace first_token_pooling
    """
    ...
    def forward(self, code_hidden, text_hidden):
        ...
        pool_text_hidden = self.text_pooler(text_hidden)
        multi_hidden = torch.mul(pool_code_hidden,pool_text_hidden)
        diff_hidden = torch.abs(pool_text_hidden - pool_text_hidden)
        concated_hidden = torch.cat((multi_hidden, diff_hidden), 1)
        x = self.dropout(concated_hidden)
        x = self.dense(x)
        x = torch.tanh(x)
        x = self.dropout(x)
...
class RelationClassifyHeader5(nn.Module):
    """
    H2:
    use averaging pooling across tokens to replace first_token_pooling
    """
    ...
    def forward(self, code_hidden, text_hidden):
        ...
        multi_hidden = torch.mul(pool_code_hidden,pool_text_hidden)
        diff_hidden = torch.abs(pool_text_hidden - pool_text_hidden)
        concated_hidden = torch.cat((pool_text_hidden, pool_code_hidden, diff_hidden), 1)
        x = self.dropout(concated_hidden)
        x = self.dense(x)
        x = torch.tanh(x)
        x = self.dropout(x)
...
class RelationClassifyHeader6(nn.Module):
    """
    H2:
    use averaging pooling across tokens to replace first_token_pooling
    """
    ...
    def forward(self, code_hidden, text_hidden):
        ...
        multi_hidden = torch.mul(pool_code_hidden,pool_text_hidden)
        diff_hidden = torch.abs(pool_text_hidden - pool_text_hidden)
        concated_hidden = torch.cat((pool_text_hidden, pool_code_hidden, diff_hidden, multi_hidden), 1)
        x = self.dropout(concated_hidden)
        x = self.dense(x)
        x = torch.tanh(x)
        x = self.dropout(x)
...
class RelationClassifyHeader7(nn.Module):
    """
    H2:
    use averaging pooling across tokens to replace first_token_pooling
    """
    ...
    def forward(self, code_hidden, text_hidden):
        ...
        multi_hidden = torch.mul(pool_code_hidden,pool_text_hidden)
        diff_hidden = torch.abs(pool_text_hidden - pool_text_hidden)
        concated_hidden = torch.cat((pool_text_hidden, pool_code_hidden, multi_hidden), 1)
        x = self.dropout(concated_hidden)
        x = self.dense(x)
        x = torch.tanh(x)
        x = self.dropout(x)
...
class TBertT1(TwinBert):
    def __init__(self, config, code_bert):
        ...
        self.cls = RelationClassifyHeader1(config)
...
class TBertT3(TwinBert):
    def __init__(self, config, code_bert):
        ...
        self.cls = RelationClassifyHeader3(config)
...
class TBertT4(TwinBert):
    def __init__(self, config, code_bert):
        ...
        self.cls = RelationClassifyHeader4(config)
...
class TBertT5(TwinBert):
    def __init__(self, config, code_bert):
        ...
        self.cls = RelationClassifyHeader5(config)
...
class TBertT6(TwinBert):
    def __init__(self, config, code_bert):
        ...
        self.cls = RelationClassifyHeader6(config)
...
class TBertT7(TwinBert):
    def __init__(self, config, code_bert):
        ...
        self.cls = RelationClassifyHeader7(config)
