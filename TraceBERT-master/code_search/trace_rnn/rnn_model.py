...
class classifyHeader1(nn.Module):
    def __init__(self, hidden_size, rnn_type):
        super().__init__()
        bi = 1
        if rnn_type == 'bi_gru':
        ...
        self.hidden_size = hidden_size
        self.sigmoid = nn.Sigmoid()
        self.dense = nn.Linear(hidden_size * 2 * bi, hidden_size * 1 * bi)
        self.dropout = nn.Dropout(0.2)
        self.output_layer = nn.Linear(hidden_size * 1 * bi, 2)

    def forward(self, nl_hidden, pl_hidden):
        concated_hidden = torch.cat((nl_hidden, pl_hidden), 1)
        fuse_hidden = self.dense(concated_hidden)
        sigmoid_hidden = self.dropout(self.sigmoid(fuse_hidden))
        logits = self.output_layer(sigmoid_hidden)
        return logits
    
class classifyHeader2(nn.Module):
    def __init__(self, hidden_size, rnn_type):
        super().__init__()
        bi = 1
        if rnn_type == 'bi_gru':
        ...
        self.hidden_size = hidden_size
        self.sigmoid = nn.Sigmoid()
        self.dense = nn.Linear(hidden_size * 1 * bi, hidden_size * 2 * bi)
        self.dropout = nn.Dropout(0.2)
        self.output_layer = nn.Linear(hidden_size * 2 * bi, 2)

    def forward(self, nl_hidden, pl_hidden):
        diff_hidden = torch.abs(nl_hidden - pl_hidden)
        fuse_hidden = self.dense(diff_hidden)
        sigmoid_hidden = self.dropout(self.sigmoid(fuse_hidden))
        logits = self.output_layer(sigmoid_hidden)
        return logits
    
class classifyHeader3(nn.Module):
    def __init__(self, hidden_size, rnn_type):
        super().__init__()
        bi = 1
        if rnn_type == 'bi_gru':
    ...
    def forward(self, nl_hidden, pl_hidden):
        ...
        multi_hidden = torch.mul(nl_hidden, pl_hidden)
        fuse_hidden = self.dense(multi_hidden)
        sigmoid_hidden = self.dropout(self.sigmoid(fuse_hidden))
        logits = self.output_layer(sigmoid_hidden)
        return logits
    
class classifyHeader4(nn.Module):
    def __init__(self, hidden_size, rnn_type):
        super().__init__()
        bi = 1
        if rnn_type == 'bi_gru':
        ...
        self.hidden_size = hidden_size
        self.sigmoid = nn.Sigmoid()
        self.dense = nn.Linear(hidden_size * 2 * bi, hidden_size * 2 * bi)
        self.dropout = nn.Dropout(0.2)
        self.output_layer = nn.Linear(hidden_size * 2 * bi, 2)
...
class classifyHeader5(nn.Module):
    def __init__(self, hidden_size, rnn_type):
        super().__init__()
        bi = 1
        if rnn_type == 'bi_gru':
        ...
        self.hidden_size = hidden_size
        self.sigmoid = nn.Sigmoid()
        self.dense = nn.Linear(hidden_size * 3 * bi, hidden_size * 2 * bi)
        self.dropout = nn.Dropout(0.2)
        self.output_layer = nn.Linear(hidden_size * 2 * bi, 2)
    ...
    def forward(self, nl_hidden, pl_hidden):
        concated_hidden = torch.cat((nl_hidden, pl_hidden), 1)
        diff_hidden = torch.abs(nl_hidden - pl_hidden)
        fuse_hidden = torch.cat((diff_hidden, concated_hidden), 1)
        fuse_hidden = self.dense(fuse_hidden)
        sigmoid_hidden = self.dropout(self.sigmoid(fuse_hidden))
        logits = self.output_layer(sigmoid_hidden)
        return logits
    
class classifyHeader6(nn.Module):
    def __init__(self, hidden_size, rnn_type):
        super().__init__()
        bi = 1
        if rnn_type == 'bi_gru':
...
class classifyHeader7(nn.Module):
    def __init__(self, hidden_size, rnn_type):
        super().__init__()
        bi = 1
        if rnn_type == 'bi_gru':
    ...
    def forward(self, nl_hidden, pl_hidden):
        ...
        fuse_hidden = torch.cat((multi_hidden, concated_hidden), 1)
        fuse_hidden = self.dense(fuse_hidden)
        sigmoid_hidden = self.dropout(self.sigmoid(fuse_hidden))
        logits = self.output_layer(sigmoid_hidden)
        return logits
...
class RNNTracer1(nn.Module):
    def __init__(self, hidden_dim, embd_info, embd_trainable, is_no_padding, max_seq_len=128, rnn_type='bi_gru'):
        ...
        self.pl_encoder = RNNEncoder(
            hidden_dim, self.embd_info, max_seq_len, embd_trainable, is_no_padding, rnn_type)
        self.cls = classifyHeader1(hidden_dim, rnn_type)
...
class RNNTracer2(nn.Module):
    def __init__(self, hidden_dim, embd_info, embd_trainable, is_no_padding, max_seq_len=128, rnn_type='bi_gru'):
        ...
        self.pl_encoder = RNNEncoder(
            hidden_dim, self.embd_info, max_seq_len, embd_trainable, is_no_padding, rnn_type)
        self.cls = classifyHeader2(hidden_dim, rnn_type)
...
class RNNTracer3(nn.Module):
    def __init__(self, hidden_dim, embd_info, embd_trainable, is_no_padding, max_seq_len=128, rnn_type='bi_gru'):
        ...
        self.pl_encoder = RNNEncoder(
            hidden_dim, self.embd_info, max_seq_len, embd_trainable, is_no_padding, rnn_type)
        self.cls = classifyHeader3(hidden_dim, rnn_type)
...
class RNNTracer4(nn.Module):
    def __init__(self, hidden_dim, embd_info, embd_trainable, is_no_padding, max_seq_len=128, rnn_type='bi_gru'):
        ...
        self.pl_encoder = RNNEncoder(
            hidden_dim, self.embd_info, max_seq_len, embd_trainable, is_no_padding, rnn_type)
        self.cls = classifyHeader4(hidden_dim, rnn_type)
...
class RNNTracer5(nn.Module):
    def __init__(self, hidden_dim, embd_info, embd_trainable, is_no_padding, max_seq_len=128, rnn_type='bi_gru'):
        ...
        self.pl_encoder = RNNEncoder(
            hidden_dim, self.embd_info, max_seq_len, embd_trainable, is_no_padding, rnn_type)
        self.cls = classifyHeader5(hidden_dim, rnn_type)
...
class RNNTracer6(nn.Module):
    def __init__(self, hidden_dim, embd_info, embd_trainable, is_no_padding, max_seq_len=128, rnn_type='bi_gru'):
        ...
        self.pl_encoder = RNNEncoder(
            hidden_dim, self.embd_info, max_seq_len, embd_trainable, is_no_padding, rnn_type)
        self.cls = classifyHeader6(hidden_dim, rnn_type)
...
class RNNTracer7(nn.Module):
    def __init__(self, hidden_dim, embd_info, embd_trainable, is_no_padding, max_seq_len=128, rnn_type='bi_gru'):
        ...
        self.pl_encoder = RNNEncoder(
            hidden_dim, self.embd_info, max_seq_len, embd_trainable, is_no_padding, rnn_type)
        self.cls = classifyHeader7(hidden_dim, rnn_type)
